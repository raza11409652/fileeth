...
App = {
  ...
  insertProblem: async ()=>{
      ...
      const doctor = $('#doctor-id').val(); 
      if(user ==null || user <1){
        return ;
      }
      await App.Patient. insertProblem(problem, suggestion , user , App.account , doctor)
  ...
  createPatient: async()=>{
    ...
    alert('Patient Created with ID '+patientCounts);
    window.location.reload()
  },
  render: async () => {
      ...
      //  $('#reports-view').html('');
       if(userRef == id){
         ...
         $('#reports-view').append(`
            ...
            <span class="float-right">Doctor ID:${doctor}</span>
              <h3>${reportid}</h3>
              <p>${content}</p>
              <p>${suggestion}</p>
            </div>
        ...
        // $('#reports-view').html(`
        //   <div class='alert alert-danger'>No Report found</div>
        // `);
       }
      }
  } ,
...
function findPatient(){
  ...
  var  data =  filter(resultArray , value) ; 
 console.log(data);
  ...
  for(var i=0;i<length ; i++){
      const id = data[i][0].c[0];
      const name = data[i][1];
      const age = data[i][2].c[0];
      const createdBy = data[i][4];
      record.append( `<tr>
          <td>${id}</td>
          <td>${name}</td>
          <td>${age}</td>
